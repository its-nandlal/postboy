...
function TabBar() {
  ...
  return (
    <>
      <div className='flex items-center border-b border-zinc-800 bg-zinc-900 overflow-x-auto'>
        {
          tabs.map((tab)=>(
            <div
            key={tab.id}
            ...
            : "text-zinc-400 hover:text-white"}`}>
              ...
              <span className={`font-semibold ${requestColorMap[tab.method] || "text-gray-500"}`}>{tab.method}</span>
              <p className='max-w-xs truncate font-semibold flex items-center gap-1'>
                {tab.title}
                {tab.unsavedChanges && (
                  <span className='text-3xl leading-0 text-red-500 pb-1 group-hover:hidden transition-all ease-in-out'>•</span>
                )}
              </p>
        ...
        <Button 
        onClick={addTab}
        className='!px-2.5 py-1 text-zinc-900 hover:text-zinc-800'>
          <Plus/>
        </Button>
      </div>
      {
