...
interface SaveRequsetToCollectionModalProps {
    isModalOpen: boolean;
...
export default function SaveRequsetToCollectionModal ({
    isModalOpen, setIsModalOpen, requestData = {
        name: "Untitled",
        url: "https://echo.hoppscotch.io",
        method: REST_METHOD.GET
    },
    initialName = "Untiteld",
    collectionId
}: SaveRequsetToCollectionModalProps) {
...
// Or consider skipping the call altogether if workspaceId is falsy, depending on how your hook handles it
  ...
  useEffect(() => {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        toast.success(`Requset saved to "${selectedCollection?.name}" collection`)
    ...
    return (
        ...
        submitVariant="default">
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2 text-zinc-200">Requset name</label>
                <div className="relative">
                  ...
                  className="absolute right-3 top-1/2 transform -translate-y-1/2">
                    <span
                    className={`text-sm font-bold px-2 py-1 rounded-sm ${requestColorMap[requestData.method]} bg-zinc-700`}>
                      {requestData.method}
                    </span>
                  </div>
                </div>
              ...
              <div>
                ...
                <div
                className="flex items-center space-x-2 text-sm text-zinc-400 mb-3">
                    ...
                    filteredCollections.map(collection => (
                      ...
                      : "hover:bg-zinc-800 border border-transparent"}`}>
                        <div className="flex items-center space-x-3">
                          ...
                          <span className={`text-sm font-medium ${selectedCollectionId === collection.id 
                            ? "text-indigo-200" : "text-zinc-200"}`}>
                            {collection.name}
                          </span>
                        </div>
